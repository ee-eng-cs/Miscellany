...
echo [E] keytool - list keystore 'testkeys'
echo [F] keytool - list truststore 'samplecacerts'
...
if /i "%KEY:~0,1%"=="A" (
  ...
  set LABEL=[E] keytool - list keystore 'testkeys'
  call :EEE
) else if /i "%KEY%"=="F" (
  set LABEL=[F] keytool - list truststore 'samplecacerts'
  call :FFF
) else (
  goto :eof
)
...
:EEE
cls
%JAVA_HOME%\bin\keytool -list -v -keystore ..\src\main\resources\security\testkeys -storepass passphrase
call :RedLabelAndPause
cls
goto :eof
...
:FFF
cls
%JAVA_HOME%\bin\keytool -list -v -keystore ..\src\main\resources\security\samplecacerts -storepass changeit -alias localhost
echo.
%JAVA_HOME%\bin\keytool -list -v -keystore ..\src\main\resources\security\samplecacerts -storepass changeit -alias verisignclass1ca
call :RedLabelAndPause
cls
goto :eof
